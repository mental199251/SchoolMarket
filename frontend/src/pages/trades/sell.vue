<template>
  <view class="page">
    <view class="header">
      ...
      <text class="title">收到的请求</text>
      <button class="nav-button" @click="goProducts">我的商品</button>
    </view>
    ...
    <view v-if="loading && trades.length === 0" class="empty">正在加载交易请求</view>
    <view v-else-if="trades.length === 0" class="empty">暂无交易请求</view>
    ...
    <view v-for="trade in trades" :key="trade.id" class="trade-card">
      <view class="trade-main" @click="goProduct(trade.product_id)">
        ...
        <view class="trade-copy">
          ...
          <text class="trade-meta">买家：{{ trade.buyer?.nickname || trade.buyer?.username || '-' }}</text>
...
export default {
  ...
  methods: {
    ...
    async fetchTrades() {
      this.loading = true
      try {
        const data = await getMySellTrades(this.filters)
